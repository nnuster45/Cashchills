...
import {
  FiTrendingDown, FiEdit2, FiTrash2, FiLoader, FiSearch,
  FiX, FiPackage, FiPlus, FiCheck, FiCheckCircle,
  FiChevronLeft, FiChevronRight, FiMail, FiClock,
  FiFileText, FiPaperclip, FiExternalLink, FiStar,
} from 'react-icons/fi'
...
function getTransactionVisual(tx: Transaction, mode: 'pending' | 'history' | 'detail' = 'history'): TransactionVisual {
  ...
  const bankDetail = tx?.notes || tx?.email_subject || tx?.category || 'Bank notification'
  const platformDetail = tx?.notes || tx?.merchant || tx?.category || 'Platform payout'
  ...
  if (lookup.includes('grab')) {
    return { src: '/logos/grab.png', alt: 'Grab', sourceLabel: mode === 'pending' ? 'Grab payout' : 'Grab', detailLabel: platformDetail }
  }
  if (lookup.includes('line') || lookup.includes('lineman')) {
    return { src: '/logos/lineman.png', alt: 'Lineman', sourceLabel: mode === 'pending' ? 'Lineman payout' : 'Lineman', detailLabel: platformDetail }
  }
  if (lookup.includes('shopee')) {
    return { src: '/logos/shopee.png', alt: 'Shopee', sourceLabel: mode === 'pending' ? 'Shopee payout' : 'Shopee', detailLabel: platformDetail }
  }
  return {
...
export default function ExpenseSection({ transactions, categories, onUpdate, onDelete, onAddTransaction, onAddCategory, loading }: ExpenseSectionProps) {
  const safeTx = Array.isArray(transactions) ? transactions : []
  const safeCats = Array.isArray(categories) ? categories : []

  const expenseTx = safeTx
    .filter((t) => t?.type === 'expense')
    .sort((a, b) => new Date(b?.date ?? 0).getTime() - new Date(a?.date ?? 0).getTime())

  const needsReview = expenseTx.filter((t) => t?.needs_review && t?.source === 'email')
  const confirmed = expenseTx.filter((t) => !t?.needs_review || t?.source !== 'email')
  ...
  function openConfirmDialog(tx: Transaction) {
    ...
    const existingItems = Array.isArray(tx?.items) && tx.items.length > 0
      ? tx.items.map((item) => ({
          name: item.name,
      ...
      : [{ name: '', amount: '', quantity: '1', notes: '' }]
    setConfirmItems(existingItems)
  }
  ...
  function handleConfirmWithCategory() {
    ...
    onUpdate({
      ...
      items: validItems.length > 0 ? validItems : confirmTx.items,
    })
    setConfirmTx(null)
    setConfirmCategory('')
    setConfirmMerchant('')
    ...
    return (
      <div className="space-y-5 pb-24">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-gray-800">รายละเอียดรายจ่าย</h2>
        </div>

        <div className="rounded-3xl overflow-hidden shadow-lg" style={{ background: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(20px)' }}>
          <div className="p-6" style={{ background: 'linear-gradient(135deg, hsl(0 75% 58%) 0%, hsl(350 70% 48%) 100%)' }}>
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
                <FiTrendingDown className="h-7 w-7 text-white" />
              </div>
              ...
              <div className="space-y-1">
                {items.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-b-0">
                    ...
                    <span className="text-sm font-bold text-red-500 ml-3">{formatAmount(item.amount * (item.quantity || 1))} THB</span>
                  </div>
                ))}
                <div className="flex items-center justify-between pt-3 mt-2 border-t border-gray-200">
                  ...
                  <span className="text-base font-extrabold text-red-500">{formatAmount(items.reduce((sum, i) => sum + i.amount * (i.quantity || 1), 0))} THB</span>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-5 pb-24">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[24px] font-extrabold tracking-tight text-slate-800">รายจ่าย</h2>
          <p className="text-xs font-medium text-slate-400">Expenses</p>
        </div>
        <Button size="sm" className="h-10 gap-1.5 rounded-[10px] bg-red-500 px-4 hover:bg-red-600" onClick={onAddTransaction}>
        ...
        <div className="mb-5 flex items-start gap-3">
          <div className="flex items-center gap-2 text-[13px] text-slate-500">
            ...
            <span className="font-medium">
              {needsReview.length > 0 ? `${needsReview.length} รายการที่ยังไม่ยืนยัน` : `${expenseTx.length} รายการทั้งหมด`}
            </span>
          </div>
        </div>
              ...
              >{(() => {
                ...
                <div className="relative z-10 flex justify-end">
                  <div className="text-right">
                    ...
                    <p className="mt-1 text-[16px] font-extrabold text-red-500">-{formatAmount(tx?.amount ?? 0)} THB</p>
                ...
                <div className="absolute bottom-3 right-3 z-10 flex gap-2">
                  <button
                    ...
                    className="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-[8px] bg-emerald-500 px-3.5 text-[13px] font-semibold text-white transition-colors hover:bg-emerald-600"
                  >
                    ยืนยัน
                  </button>
                  <button
      ...
      <div>
        ...
        {filteredConfirmed.length === 0 && filteredReview.length === 0 ? (
          <div className="rounded-[12px] border border-slate-100 bg-white p-8 text-center shadow-[0_10px_24px_rgba(15,23,42,0.045)]">
            <FiTrendingDown className="h-12 w-12 mx-auto mb-3 text-red-200" />
            <p className="text-sm font-medium text-gray-500">ยังไม่มีรายจ่าย</p>
            <p className="text-xs text-gray-400 mt-1">เพิ่มรายจ่ายหรือ sync จากอีเมล</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              >{(() => {
                ...
                <div className="flex h-[118px] flex-col pl-[140px]">
                  <div className="flex min-h-0 flex-1 flex-col items-end text-right">
                    ...
                    <p className="mt-2 text-[16px] font-extrabold text-red-500">-{formatAmount(tx?.amount ?? 0)} THB</p>
                          ...
                          <>
                            ...
                            {tx.gross_amount != null && tx.gross_amount > 0 && (
                              <div className="flex justify-between items-center text-[10px] mt-1 pt-1 border-t border-slate-200">
                                <span className="text-red-400 font-medium">รวมทั้งสิ้น</span>
                                <span className="font-bold text-red-500">-{formatAmount(tx.gross_amount)}</span>
                              </div>
                            )}
                          </>
                        ) : (
              ...
              <div className="space-y-4">
                <div className="rounded-[14px] border border-slate-100 bg-white p-3.5 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
                  <div className="flex gap-4">
                    <TransactionLogo
                      tx={confirmTx}
                      className="flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-[14px] border border-red-100 bg-white p-2"
                    />
                            ...
                            <>
                              ...
                              {confirmTx.gross_amount != null && confirmTx.gross_amount > 0 && (
                                <div className="flex justify-between items-center text-[11px] mt-1 pt-1 border-t border-slate-200">
                                  <span className="text-red-400 font-medium">รวมทั้งสิ้น</span>
                                  <span className="font-bold text-red-500">-{formatAmount(confirmTx.gross_amount)}</span>
                                </div>
                              )}
                            </>
                          ) : (
                ...
                <div>
                  ...
                  {showAddCatConfirm ? (
                        ...
                        <Input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              ...
                              setAddingCat(true)
                              onAddCategory({ name: newCatName.trim(), type: 'expense', is_default: false }).then((cat) => {
                    ...
                    <Select value={confirmCategory} onValueChange={(val) => {
                      ...
                      <SelectContent>
                        {safeCats.filter((c) => c?.type === 'expense').map((c) => (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button variant="outline" size="sm" className="h-7 text-xs gap-1 rounded-lg" onClick={addConfirmItem}>
                      <FiPlus className="h-3 w-3" /> เพิ่มรายการ
                    </Button>
                  </div>
                  <div className="space-y-2">
                    {confirmItems.map((item, idx) => (
                      <div key={idx} className="rounded-xl border bg-gray-50/80 p-3 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] font-semibold text-gray-400">รายการ #{idx + 1}</span>
                          {confirmItems.length > 1 && (
                            <button onClick={() => removeConfirmItem(idx)} className="p-1 rounded hover:bg-red-50 text-red-400 hover:text-red-600">
                        ...
                        <Input
                          ...
                          value={item.name}
                          onChange={(e) => updateConfirmItem(idx, 'name', e.target.value)}
                        ...
                        <div className="flex gap-2">
                          <div className="flex-1">
                            <Input
                              ...
                              placeholder="ราคา"
                              value={item.amount}
                              onChange={(e) => updateConfirmItem(idx, 'amount', e.target.value)}
                              className="h-9 text-sm rounded-lg"
                            />
                          </div>
                          <div className="w-20">
                            <Input
                              ...
                              placeholder="จำนวน"
                              value={item.quantity}
                              onChange={(e) => updateConfirmItem(idx, 'quantity', e.target.value)}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <Button
                    ...
                    className="flex-1 rounded-xl gap-1.5 bg-red-500 hover:bg-red-600"
                  >
                    <FiCheck className="h-4 w-4" /> ยืนยัน
                  </Button>
                </div>
        ...
        <DialogContent className="rounded-3xl">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleSaveEdit} className="rounded-xl bg-red-500 hover:bg-red-600">บันทึก</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <button
        ...
        style={{ background: 'linear-gradient(135deg, hsl(0 75% 58%), hsl(350 70% 48%))' }}
      >
